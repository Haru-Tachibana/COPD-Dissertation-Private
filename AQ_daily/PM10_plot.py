...
PM104 = pd.read_csv(file_path + '/PM10_2024.csv')
...
def plot_counties_for_year(df, year, output_path):
    ...
    for county, color in zip(counties, colors):
        ...
        plt.scatter(county_data['DayOfYear'], 
                   county_data['Daily Mean PM10 Concentration'],
                   label=county,
                   color=color,
                   alpha=0.6,
                   s=30)
        ...
        for i in range(len(county_data_sorted) - 1):
            if county_data_sorted.iloc[i+1]['DayOfYear'] - county_data_sorted.iloc[i]['DayOfYear'] == 1:
                plt.plot([county_data_sorted.iloc[i]['DayOfYear'], 
                        ...
                        [county_data_sorted.iloc[i]['Daily Mean PM10 Concentration'], 
                         county_data_sorted.iloc[i+1]['Daily Mean PM10 Concentration']],
                        color=color,
                        alpha=0.4)
    
    plt.xlabel('Day of Year')
    plt.ylabel('PM10 Concentration (ug/m3)')
    plt.title(f'Daily Mean PM10 Concentration by County ({year})')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.3)
    
    # Add year average line
    year_avg = df['Daily Mean PM10 Concentration'].mean()
    ...
    # Save plot
    plt.savefig(os.path.join(output_path, f'PM10_concentration_by_county_{year}.png'), 
                bbox_inches='tight',
                dpi=300)
    plt.close()
...
def print_county_statistics(df, year):
    ...
    overall_mean = df['Daily Mean PM10 Concentration'].mean()
    overall_max = df['Daily Mean PM10 Concentration'].max()
    overall_min = df['Daily Mean PM10 Concentration'].min()
    ...
    for county in counties:
        ...
        mean_conc = county_data['Daily Mean PM10 Concentration'].mean()
        max_conc = county_data['Daily Mean PM10 Concentration'].max()
        print(f"\n{county}:")

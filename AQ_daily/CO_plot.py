...
# Read the data
CO8 = pd.read_csv(file_path + '/CO_2018.csv')
CO9 = pd.read_csv(file_path + '/CO_2019.csv')
CO0 = pd.read_csv(file_path + '/CO_2020.csv')
...
def plot_counties_for_year(df, year, output_path):
    ...
    for county, color in zip(counties, colors):
        ...
        plt.scatter(county_data['DayOfYear'], 
                   county_data['Daily Max 8-hour CO Concentration'],
                   label=county,
                   color=color,
                   alpha=0.6,
                   s=30)
        ...
        for i in range(len(county_data_sorted) - 1):
            if county_data_sorted.iloc[i+1]['DayOfYear'] - county_data_sorted.iloc[i]['DayOfYear'] == 1:
                plt.plot([county_data_sorted.iloc[i]['DayOfYear'], 
                        ...
                        [county_data_sorted.iloc[i]['Daily Max 8-hour CO Concentration'], 
                         county_data_sorted.iloc[i+1]['Daily Max 8-hour CO Concentration']],
                        color=color,
                        alpha=0.4)
    
    plt.xlabel('Day of Year')
    plt.ylabel('CO Concentration (ppm)')
    plt.title(f'Daily Maximum 8-hour CO Concentration by County ({year})')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.3)
    
    # Add year average line
    year_avg = df['Daily Max 8-hour CO Concentration'].mean()
    ...
    # Save plot
    plt.savefig(os.path.join(output_path, f'CO_concentration_by_county_{year}.png'), 
                bbox_inches='tight',
                dpi=300)
    plt.close()
...
def print_county_statistics(df, year):
    ...
    overall_mean = df['Daily Max 8-hour CO Concentration'].mean()
    overall_max = df['Daily Max 8-hour CO Concentration'].max()
    overall_min = df['Daily Max 8-hour CO Concentration'].min()
    ...
    for county in counties:
        ...
        mean_conc = county_data['Daily Max 8-hour CO Concentration'].mean()
        max_conc = county_data['Daily Max 8-hour CO Concentration'].max()
        print(f"\n{county}:")
...
dataframes = [CO8, CO9, CO0, CO1, CO2, CO3, CO4]

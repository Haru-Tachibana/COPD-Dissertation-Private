...
for name, (model, params) in models.items():
    (results[name], best_models[name]) = evaluate_model(model, params, X_train_scaled, X_test_scaled, y_train, y_test)
    print(f"\n{name}:")
...
# Prepare data for plotting if feature importances exist
if feature_importances:
    importance_df = pd.DataFrame(feature_importances)
    importance_df.plot(kind='bar', stacked=False, figsize=(15, 8))
    plt.title('Feature Importances Across Tree-based Models')
    plt.xlabel('Features')
